...
public class Exercise04_HotelReservation {
    ...
    public double calculateStayTotal(int numberOfNights) {
        ...
        if (numberOfNights < MINIMUM_NIGHTS_FOR_DISCOUNT_RATE) {
            totalHotelCost = DAILY_RATE * numberOfNights;
            return totalHotelCost;
        } else if (numberOfNights >= MINIMUM_NIGHTS_FOR_DISCOUNT_RATE) {
            totalHotelCost = DISCOUNT_RATE * numberOfNights;
            return totalHotelCost;
        } else {
            return 0.0;
        }
    ...
    public double calculateStayTotal(int numOfTotalNights, boolean includesParking) {
        ...
        if (numOfTotalNights < MINIMUM_NIGHTS_FOR_DISCOUNT_RATE && includesParking) {
            totalHotelCost = numOfTotalNights * (DAILY_RATE + PARKING_RATE);
            return totalHotelCost;
        } else if (numOfTotalNights < MINIMUM_NIGHTS_FOR_DISCOUNT_RATE) {
            totalHotelCost = numOfTotalNights * DAILY_RATE;
            return totalHotelCost;
        } else if ( numOfTotalNights >= MINIMUM_NIGHTS_FOR_DISCOUNT_RATE && includesParking) {
            totalHotelCost = numOfTotalNights * (DISCOUNT_RATE + PARKING_RATE);
            return totalHotelCost;
        } else if (numOfTotalNights >= MINIMUM_NIGHTS_FOR_DISCOUNT_RATE) {
            totalHotelCost = numOfTotalNights * DISCOUNT_RATE;
            return totalHotelCost;
        }
    return 0.0;
    }
    ...
    public double calculateStayTotal(int numOfTotalNights, boolean includesParking, boolean includesLateCheckout) {
        double hotelCostWithParkingAndLateFee;
        if (includesLateCheckout) {
            hotelCostWithParkingAndLateFee = calculateStayTotal(numOfTotalNights, includesParking) + LATE_CHECKOUT_FEE;
            return hotelCostWithParkingAndLateFee;
        }
        return calculateStayTotal(numOfTotalNights, includesParking);
    }
}
