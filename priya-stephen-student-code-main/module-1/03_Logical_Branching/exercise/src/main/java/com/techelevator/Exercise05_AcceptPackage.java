...
public class Exercise05_AcceptPackage {
    ...
    public boolean acceptPackage(int weightPounds, int lengthInches, int widthInches, int heightInches) {
        int cubicInches;
         ...
         if (weightPounds <= 40 && cubicInches <= 6912) {
             return true;
         }
    ...
    public boolean acceptPackage(int weightPounds, int lengthInches, int widthInches, int heightInches, boolean isSurchargePaid) {
        if (acceptPackage( weightPounds, lengthInches, widthInches, heightInches) && lengthInches < 67 && widthInches < 67 &&
                heightInches < 67 && !isSurchargePaid) {
            return true;
        } else if (acceptPackage(weightPounds, lengthInches, widthInches, heightInches) && (lengthInches > 66 || widthInches > 66 || heightInches > 66)
                && isSurchargePaid) {
            return true;
        }
        return false;
