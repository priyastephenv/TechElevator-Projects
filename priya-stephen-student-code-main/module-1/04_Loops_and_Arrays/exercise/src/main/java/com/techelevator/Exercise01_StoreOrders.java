...
public class Exercise01_StoreOrders {
    ...
    public int getCheesePizzaRevenue(int[] orders) {
        int totalSmallCheesePizzas = 0;
        int totalMediumCheesePizzas = 0;
        int totalLargeCheesePizzas = 0;
        int smallCheeseMoney = 8;
        int mediumCheeseMoney = 11;
        int largeCheeseMoney = 14;
        int totalZaCash;
        for (int j = 0; j < orders.length; j++) {
            if (orders [j] == SMALL_CHEESE) {
                totalSmallCheesePizzas +=1;
            } else if (orders [j] == MEDIUM_CHEESE) {
                    totalMediumCheesePizzas +=1;
            } else if (orders [j] == LARGE_CHEESE) {
                totalLargeCheesePizzas +=1;
            }
        }
        totalZaCash = ( (totalSmallCheesePizzas * smallCheeseMoney) + (totalMediumCheesePizzas * mediumCheeseMoney)
        + (totalLargeCheesePizzas * largeCheeseMoney));
        return totalZaCash;
    }
}
