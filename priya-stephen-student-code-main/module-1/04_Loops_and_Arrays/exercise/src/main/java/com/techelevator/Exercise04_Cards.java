...
public class Exercise04_Cards {
    ...
    */
    ...
    public String[] discardFirstCard(String[] hand) {
        String[] fourCardHand = new String[hand.length - 1];
        for (int i = 0; i < hand.length - 1; i++) {
            fourCardHand[i] = hand[i + 1];
        }
        return fourCardHand;
    }
    ...
    public String[] discardTopCard(String[] remainingDeck) {
        if (remainingDeck.length >= 1) {
            String[] afterFirstDiscard = new String[remainingDeck.length - 1];
            for (int i = 0; i < remainingDeck.length - 1; i++) {

                afterFirstDiscard[i] = remainingDeck[i + 1];
            }
            return afterFirstDiscard;
        } else {
