...
public class Exercise05_Weather {
    ...
    */
    public int hottestDay(int[] dailyHighs) {
        int highestDailyHigh = dailyHighs [0];
        if (dailyHighs.length == 1) {
            return dailyHighs[0];

        } else  {
            for (int i = 1; i < dailyHighs.length; i++)  {
                if (dailyHighs[i] > highestDailyHigh) {
                    highestDailyHigh = dailyHighs [i];
                }

            }
        }

        return highestDailyHigh;
    ...
    public int[] fixTemperatures(int[] temperatures) {
        int [] updatedTemperatures = new int [temperatures.length];
        for (int i = 0; i < temperatures.length; i++) {
            if (i % 2 ==0) {
            updatedTemperatures [i] = (temperatures [i] + 2);
            } else {
                updatedTemperatures [i] = temperatures [i];
            }
        }
