...
public class MemoryReservationDao implements ReservationDao {
    ...
    public Reservation createReservation(Reservation reservation) {
        ...
        /*
         * Check the hotel id from the reservation exists
         */
        List<Hotel> hotels = hotelDao.getHotels();
        int reservationHotelId = reservation.getHotelId();
        boolean hotelIdFound = false;

        for(Hotel eachHotel : hotels){
            int hotelId = eachHotel.getId();

            if(hotelId == reservationHotelId){
                // SUCCESS: Found a matching hotel id from the hotels list
                hotelIdFound = true;
                break;
            }
        }
        ...
        if(!hotelIdFound){
            throw new DaoException("Hotel id from reservation NOT FOUND, " + reservationHotelId);
        }
