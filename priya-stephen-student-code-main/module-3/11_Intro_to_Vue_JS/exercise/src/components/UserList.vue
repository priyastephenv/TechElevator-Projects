  ...
  computed: {
   ...
   filteredList () {
    ...
    if (this.search.firstName != "") {
      filtered = filtered.filter((user)=> user.firstName.toLowerCase().includes(this.search.firstName.toLowerCase()) )
    }
    if (this.search.lastName != "") {
      filtered = filtered.filter((user)=>user.lastName.toLowerCase().includes(this.search.lastName.toLowerCase()) )
    }
  if (this.search.username != "") {
    filtered = filtered.filter((user) =>user.username.toLowerCase().includes(this.search.username.toLowerCase()))
  }
  if (this.search.emailAddress != "") {
    filtered = filtered.filter((user) =>user.emailAddress.toLowerCase().includes(this.search.emailAddress.toLowerCase()))
  }

  if (this.search.status != "") {
    filtered = filtered.filter((user) =>user.status===this.search.status)
  }
