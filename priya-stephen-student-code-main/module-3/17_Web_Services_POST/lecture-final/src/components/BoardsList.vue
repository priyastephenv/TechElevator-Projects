...
export default {
  ...
  methods: {
    ...
    saveNewBoard() {
      this.isLoading = true;
      boardsService.addBoard(this.newBoard).then( resp => {
        ...
        // console.log(err);
        if (err.response) {
          alert("Error adding Board: " + err.response.status + " " + err.response.statusText);
        } else if (err.request) {
          alert("Error adding Board: " + err.message);
        } else {
          alert.log('An error occurred.');
        }
        this.isLoading = false;
      })
